...
async function loadLivePrice() {
  ...
  if (!px) {
    if (livePriceEl) livePriceEl.textContent = "error";
    return;
  }

  // Обновляем цену и entry
  if (livePriceEl) livePriceEl.textContent = px.toFixed(2);
  if (entryEl) entryEl.value = px;

  // --- автозаполнение TP/SL от цены ---
  const tpEl = document.getElementById("perp_tp");
  const slEl = document.getElementById("perp_sl");

  // Пример: TP +3%, SL -3% от текущей цены
  const tpPx = px * 1.03;
  const slPx = px * 0.97;

  if (tpEl) tpEl.value = tpPx.toFixed(2);
  if (slEl) slEl.value = slPx.toFixed(2);
}

const btnFetch = document.getElementById("btn-fetch-price");
if (btnFetch) {
  btnFetch.addEventListener("click", loadLivePrice);
}
...
function calcPerp() {
  ...
  // логическая проверка TP для long/short
